...
def place_order(request):
    ...
    context = {'recaddr':recaddr,'uname':uname,'uphone':uphone,
               'listinfo':listinfo,'kit_name':'提交订单',}
...
def create_order(request):
    ...
    buycount = int(request.GET.get("buycount",'0'))
...
def buynow(request, goodsid, count):
    ...
    # order.oispay = False
    # order.otransfee = 0
    listinfo = [{"gid":goodsid,"gpic":gpic,"gtitle":gtitle,
                 "gunit":gunit,"count":count,"gprice":gprice}]
    print(str(listinfo))



    context = {'recaddr':ushou,'uphone':uphone,'uname':uname,
               'listinfo':listinfo,'buyway':"now",}
